...
struct ContentView: View {
    ...
    @State private var editingItem: CableItem?
    ...
    @ViewBuilder
    private func row(for entry: CableItem) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.name)
                .font(Theme.headlineFont)
                .foregroundColor(Theme.textPrimary)
            Text("Type: \(entry.type)  \u{2022}  Drawer: \(entry.drawer)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
        }
        .padding(.vertical, 4)
...
struct ItemEditView: View {
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Type (USB-C to HDMI)", text: $field1)
                        .focused($focusedField, equals: .field1)
                        .accessibilityIdentifier("field1Field")
                    TextField("Drawer (Drawer 2)", text: $field2)
            ...
            .navigationTitle(mode == .add ? "Add Cable" : "Edit Cable")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            store.add(name: name, type: field1, drawer: field2, notes: notes)
        case .edit(var entry):
            entry.name = name
            entry.type = field1
            entry.drawer = field2
